...
public class BookingRefundEmailConsumer {
    private final MailService mailService;

    @KafkaListener(topics = "${kafka.topic.booking-refund}", groupId = "${spring.kafka.consumer.group-id}")
    public void consume(TicketGenerationMessage message) {
        mailService.sendRefundTickets(message);
    }
}
